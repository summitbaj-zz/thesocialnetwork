...
echo ($prod['current_city']);
    ...
    <? if($my_self==0){?>
    <p style="margin-top:20px;"><? if($fcount>0){?><a href="<?=baseURL ?>u_<?=$prod['user_name']?>&rfriends"  class="btn btn-danger"  data-loading-text="Posting..." role="button">Unfriend</a><? }elseif($pcount>0){?><a href="#" class="addasfren btn btn-default disabled" role="button">Pending Request</a><? } else{ ?> <button id="<?=$prod['user_id']?>" type="button" data-loading-text="Request Sent"  class="approve-button addasfren btn btn-primary">Send Friend Request</button><? } ?> <a href="<?=baseURL ?>c_<?=$prod['user_name']?>" class="btn btn-default" role="button">Message</a></p>
    <? } ?>
